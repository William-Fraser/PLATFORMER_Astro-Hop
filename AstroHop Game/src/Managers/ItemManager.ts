...
export default class ItemManager {
    ...
    private Create() { 
        ...
        this.decideSpawned = randomMe(1, 70);
        ...
        if (created) {
            this.itemMaker.positionMe(randomMe(this.itemMaker.sprite.getBounds().width/2+100, (STAGE_HEIGHT-(this.itemMaker.sprite.getBounds().width/2))-100), -this.itemMaker.sprite.getBounds().height);
    ...
    private CheckAndRemoveOnPickUp() {
        for (let i = 0; i< this._items.length; i++){
    ...
    public SetupStart() {
        ...
        // let testingItem = new ForceField(this.stage, this.assetManager);
        // testingItem.positionMe(200, 300);
        // this._items.push(testingItem);
    }

    public Update(player:Player, score:ScoreSystem) {
        this.CheckToCreate();
